...
// Estes dois casos são divergência DELIBERADA do upstream, e a sincronização de
// 19/08/2026 os desfez em silêncio: o #3200 passou a devolver um
// unavailableLLMProvider (que falha em toda chamada) no lugar do provider
// injetado, e a sintetizar configuração de modelo a partir dos defaults quando
// o modelo não é configurado. Os dois juntos transformam "um modelo com
// problema" em "assistente mudo", que é o modo de falha caro aqui.
func TestResolvePrimaryProviderForAgent_MantemProviderInjetado(t *testing.T) {
	t.Run("provider do modelo não sobe", func(t *testing.T) {
		fallback := &mockProvider{}
		cfg := &config.Config{ModelList: []*config.ModelConfig{{
			ModelName: "quebrado",
			Provider:  "unsupported-provider",
		...
		resolved := resolvePrimaryProviderForAgent(cfg, t.TempDir(), "main", "quebrado", fallback)
		if resolved != fallback {
			t.Fatalf("devia manter o provider injetado, veio %T", resolved)
		}
	})

	t.Run("modelo sem entrada e sem credencial para herdar", func(t *testing.T) {
		// Sem entrada no model_list e sem chave nenhuma para herdar, o upstream
		// monta uma configuração a partir dos defaults: api_base padrão e
		// NENHUMA chave. O provider que sai dela responde 401 em tudo, então o
		// injetado — que é o único que fala com alguém — tem que ficar.
		//
		// Herdar de template continua valendo e é coberto pelo teste do
		// upstream logo acima: lá existe chave configurada para herdar.
		fallback := &mockProvider{}
		cfg := &config.Config{ModelList: []*config.ModelConfig{{
			ModelName: "outro",
			Model:     "outro",
			APIBase:   "https://exemplo.invalido/v1",
		}}}

		resolved := resolvePrimaryProviderForAgent(cfg, t.TempDir(), "main", "modelo-que-ninguem-configurou", fallback)
		if resolved != fallback {
			t.Fatalf("sem credencial para herdar devia manter o injetado, veio %T", resolved)
		}
	})

	t.Run("entrada explícita sem chave ainda resolve", func(t *testing.T) {
		// Endpoint local costuma não ter chave; quem está no model_list foi
		// escolhido de propósito e continua mandando.
		fallback := &mockProvider{}
		cfg := &config.Config{ModelList: []*config.ModelConfig{{
			ModelName: "local",
			Model:     "local",
		...
		if resolved := resolvePrimaryProviderForAgent(
			cfg,
			t.TempDir(),
			"main",
			"local",
			...
			t.Fatal("entrada explícita do model_list devia resolver o provider dela")
		}
	})
}
